...
final class TestKeypairsCodec: XCTestCase {
    ...
    func testArbitrarySeeds() {
        let decoded = try! XrplCodec.decodeSeed("sEdTM1uX8pu2do5XvTnutH6HsouMaM2")
        ...
        let decoded2 = try! XrplCodec.decodeSeed("sn259rEFXrQrWyx3Q7XneWcwV6dfL")
    ...
    func testEncodeWithType() {
        ...
        let decoded = try! XrplCodec.decodeSeed(edSeed)
        XCTAssertEqual(decoded.0.toHexString().uppercased(), "4C3A1D213FBDFB14C7C28D609469B341")
        XCTAssertEqual(decoded.1.algorithm.rawValue, "ed25519")
        XCTAssertEqual(try! XrplCodec.encodeSeed(decoded.0, decoded.1), edSeed)
    }
}
